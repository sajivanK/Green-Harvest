...
const GenerateFullReport = () => {
  ...
  const fetchReportData = async () => {
    ...
    try {
      setLoading(true);
      const reportResponse = await axiosApi.get(
        `/api/orders/report?startDate=${startDate}&endDate=${endDate}`,
        { withCredentials: true }
      );
      const farmerResponse = await axiosApi.get("/api/farmer/profile", {
        withCredentials: true,
      });
  ...
  const handleDownload = () => {
    ...
    const ghLogo = new Image();
    ghLogo.src = logo;

    ghLogo.onload = () => {
      doc.addImage(ghLogo, "PNG", 10, 10, 40, 40); // ✅ GreenHarvest logo left

      if (farmLogo) {
        const farmImg = new Image();
        farmImg.crossOrigin = "Anonymous";
        farmImg.src = farmLogo.startsWith("http")
          ? farmLogo
          : `http://localhost:4000/uploads/${farmLogo}`;

        farmImg.onload = () => {
          doc.addImage(farmImg, "JPEG", 160, 10, 40, 40); // ✅ Farm logo right
          renderContent();
        };
    ...
    function renderContent() {
      doc.setFontSize(18);
      doc.text("GreenHarvest - Sales Report", 105, 60, { align: "center" });

      doc.setFontSize(11);
      doc.text(`Farmer: ${farmerName}`, 15, 70);
      doc.text(`Period: ${startDate} ➔ ${endDate}`, 15, 76);
      doc.text(`Generated on: ${date}`, 15, 82);

      const totalQty = reportData.productSales.reduce(
        (acc, curr) => acc + curr.quantity,
        0
      );
      const totalRev = reportData.productSales.reduce(
      ...
      doc.setFontSize(14);
      doc.text("Top Selling Products", 15, doc.lastAutoTable.finalY + 10);
      ...
      doc.text(
        "Generated by GreenHarvest • Cleaner Earth, Greener Future",
        15,
      ...
      {reportData && (
        <div className="bg-white rounded-lg text-black p-6">
          <h3 className="text-2xl font-bold mb-2">
            GreenHarvest - Sales Report
          </h3>
          <p>
            <b>Farmer:</b> {farmerName}
          </p>
          <p>
